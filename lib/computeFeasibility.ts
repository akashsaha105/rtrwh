...
export type AssessmentInput = {
  ...
  export function computeFeasibility(assess: AssessmentInput): ReportOutput {
    ...
    const feasibilityScore = Math.round(
      0.30 * roofScore + 0.20 * openSpaceScore + 0.20 * rainfallScore + 0.15 * gwScore + 0.15 * soilScore
    );
  
    const category = feasibilityScore >= 80 ? "High" : feasibilityScore >= 50 ? "Moderate" : "Low";
    ...
    if (openSpace >= 10 && soilPerm >= 25) {
      recommendedStructures.push({ type: "Trench + multiple recharge pits", confidence: 0.9, reason: "Ample open space and decent soil infiltration." });
    } else if (openSpace >= 4 && soilPerm >= 20) {
      recommendedStructures.push({ type: "Recharge pit(s)", confidence: 0.8, reason: "Moderate open space; pits are practical." });
    } else if (openSpace < 4 && gwDepth <= 15) {
      recommendedStructures.push({ type: "Recharge shaft / well (professional)", confidence: 0.75, reason: "Limited open space, shallow groundwater favors shaft." });
    } else {
      recommendedStructures.push({ type: "Site inspection / percolation test", confidence: 0.5, reason: "Insufficient open space or low permeability; on-site test recommended." });
    }
    ...
    if (roofArea > 100 && openSpace < 4) {
      recommendedStructures.unshift({ type: "Convey runoff to community recharge or borewell (if available)", confidence: 70, reason: "Large catchment but limited on-site space." });
    }
